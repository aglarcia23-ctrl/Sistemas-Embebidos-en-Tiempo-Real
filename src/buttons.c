...
void button_task(void *pvParameters)
{
    ...
    while (1)
    {
        if (cfg->type == BUTTON_START_PAUSE)
        {
            // Imprime el valor crudo del pin una vez cada segundo (50 * 20ms)
            static int contador_log = 0;
            if (++contador_log % 50 == 0)
            {
                int lectura_real = gpio_get_level((gpio_num_t)cfg->gpio);
                ESP_LOGW(TAG, "Mirando GPIO %d... Voltaje físico detectado: %d", cfg->gpio, lectura_real);
            }
        }
